...
class TransactionService {
  ...
  add = async (block_hash: string, transactions: ITransactionData[], opts) => {
    try {
      ...
      logger.info(
        `Transaction Stored successfully, block hash: ${block_hash}`
      );

    } catch (error) {
      logger.error(`Error in store transactions for block hash : ${block_hash}, ${error}`);
      throw new Error(`Error in store transactions for block hash : ${block_hash}`)
    }
  }
  ...
  getIncompleteTransactions = async () => {
    try {
      ...
      logger.error(`Error fetching incomplete transactions`);
      throw new Error(`Error fetching incomplete transactions`);
    }
  }
}
